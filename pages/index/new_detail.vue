	...
	<view>
		<view id="newDetailContainer" v-show="order.status > 0&&isLoading">
			...
			<view class="tab-container-box" v-if="show==1">
				<view class="product-info-box">
					...
					<view class="info-text" style="font-size: 30rpx;">
						<view class="ellipsis" ><span>店铺名称：</span>
							{{product.store_name?product.store_name:'-'}}
						</view>
						<view class="ellipsis" ><span>掌柜名称：</span>{{product.shopkeeper?product.shopkeeper:'-'}}</view>
						<view class="ellipsis"><span>展示价格：</span><span style="color: #FF4B5E;">{{product.goods_search_price}}</span>元</view>
						<view class="ellipsis"><span>收货人数：</span><span style="color: #FF4B5E;">{{product.goods_position}}</span>人</view>
						<view class="ellipsis"><span>商品所在地：</span>{{product.upaddress?product.upaddress:'全国'}}</view>
					</view>
				</view>
				...
				<view class="goods-title" v-if="order.model!=6">
					<span>搜索关键词：</span> <view style="font-weight: 900;color: #FF4B5E;">{{product.product_keywrods.keywords}}</view>
				</view>
				...
				<view class="goods-title" style="border-top:12rpx solid #E2E1E0">
					...
					<view class="get-order-info">
						<view style="font-weight: 900;">{{product.platform_name}}{{product.product_platform}} <span style="color:rgb(255, 75, 94);padding-left: 20rpx;">{{product.goods_sort}}排序</span></view>
						...
						<!-- <view class="order-buyer-name">
							<span>排序方式：</span>
							<view>
								<span style="color: #FF4B5E;margin-right: 10rpx;">{{product.goods_sort}}</span>
							</view>
						</view> -->
					</view>
				</view>
				<view class="goods-title" style="border-top:12rpx solid #E2E1E0" v-if="order.model!=2">
					...
					<view>
						...
						<!-- <view style="color: #FF6600;">点击查看评价内容>>></view> -->
					</view>
				</view>
				<view style="border-top:12rpx solid #E2E1E0;padding:30rpx 24rpx;background-color:#ececec;" v-if="isLoading&&order.status<2&&order.order_expire_time > 0">
					<view class="is-item timer-box" style="justify-content: center;flex-direction: column;" >
						<view class="is-item-hd" style="font-size: 34rpx;font-weight: 900;" v-if="order.check_time==0&&product.model==4">
							上传凭证时间（<span style="color: #f00;">不可去淘宝付款</span>）:
						</view>
						<view class="is-item-hd" style="font-size: 34rpx;font-weight: 900;" v-if="order.check_time==0&&product.model!=4">
							上传凭证时间:
						</view>
						<view class="is-item-hd" style="font-size: 34rpx;font-weight: 900;" v-if="order.check_time>0&&nowDate<order.submit_time">
							回访时间（<span style="color: #f00;">不可去淘宝付款</span>）:
						</view>
						<view class="is-item-hd" style="font-size: 34rpx;font-weight: 900;" v-if="order.check_time>0&&nowDate>=order.submit_time">
			...
			<view class="tab-container-box note-box" v-if="show==3">
				<view>1、平台中接收任务的买号必须和淘宝实际浏览商品的买号一致。</view>
				<view>2、注意查看任务要求，选择正确的排序或搜索方式，请勿随意增加搜索条件。</view>
				<view>3、商家要求聊天的，请围绕商品询问至少4个相关的问题，不得一次性复制4个问题给客服，如果客服不在线，等待超过10分钟的可以直接下单。</view>
				<view>4、严禁和卖家旺旺聊天提到“刷单”、“信誉”、“任务”平台名称等敏感词语，有问题可以发起申诉。</view>
				<view>5、淘宝上实际绑定的下单地址必须和平台接任务的账号绑定的地址一致。如收货信息有变更请先修改信息后再接任务。</view>
				<view>6、淘宝不允许使用信用卡、花呗等任何信用类方式付款，不允许使用集分宝、淘金币、天猫积分等积分抵扣付款金额，否则将从本金中扣除购物金额。</view>
				<view>7、一定要等到快递真实签收后才能确认收货并按任务的评价要求给予5分好评。</view>
				<view>8、恶意提交与任务无关的图片将根据买手处罚规则进行处罚。</view>
			</view>
			<view id="footer">
		...
		methods: {
			...
			submitOrder: function() {//去做任务
				...
				if (this.order.model == 3) {
					...
					// 去操作任务
					uni.navigateTo({
						url: '/pages/index/task_operation?order_sn=' + order_sn
					});
				}
			},
