	...
	<view>
		<view id="newDetailContainer" v-show="order.status > 0&&isLoading">
			...
			<view class="tab-container-box note-box" v-if="show==3">
				<view>1、平台中接收任务的买号必须和拼多多实际浏览商品的买号一致。</view>
				...
				<view>5、拼多多上实际绑定的下单地址必须和平台接任务的账号绑定的地址一致。如收货信息有变更请先修改信息后再接任务。</view>
				<view>6、拼多多不允许使用信用卡、花呗等任何信用类方式付款，不允许使用集分宝、淘金币、天猫积分等积分抵扣付款金额，否则将从本金中扣除购物金额。</view>
		...
		methods: {
			...
			submitOrder: function() {//去做任务
				...
				if (this.order.model == 3) {
					...
					// 去操作任务
					uni.navigateTo({
						url: '/pages/index/task_operationPinDuoDuo?order_sn=' + order_sn
					});
				}
			},
