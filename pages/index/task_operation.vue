<template>
	<view id="taskOpeartion" >
		...
		<view v-show="(order.model==1&&isLoading)||(order.model==5&&isLoading)">
									...
									<view class="" style="background-color: #ececec;">
										<view class="is-list2-info has-list2-tip ">
									        <view class="" style="padding: 18rpx 24rpx;">
												<!-- <view class="has-mgt-10 has-mgb-10 is-grey is-size-15">请确认<span style="color:rgb(255, 75, 94);padding:0 10upx;">{{order.buyer_name}}</span>账号是否登录手机{{product.platform_name}}APP</view> -->
												...
												<view class="has-mgt-10 has-mgb-10 is-grey is-size-15">在当前关键词搜索下，找到<span style="font-weight: 900;color:#fe6248">目标商品</span>的同时随机点击2-3个相关的产品进行货比浏览,<span style="color:#fe6248">并在目标商品详情中找到以下问题的答案</span> <navigator url="/pages/detail/content?id=4" hover-class="navigator-hover">。<span class="is-blue">→点击查看示例</span></navigator></view>
												...
												<view class="has-mgt-10 has-mgb-10 is-grey is-size-15">在目标商品店铺，随机选择店铺几款产品进行浏览。<navigator url="/pages/detail/content?id=5" hover-class="navigator-hover"><span class="is-blue">→点击查看示例</span></navigator></view>
									        </view>
									    </view>
									</view>
								</view>
			...
			<view style="margin-top: 20rpx;" class="step-two" >
				...
				<view style="padding: 18rpx 24rpx;background-color: #fff;">
					<view class='tag-box-title' style="display: flex;align-items: center;">
						...
						<view style="font-size: 30rpx;color: #333;">下单要求</view>
					</view>
					<view style="display: flex;align-items: center;flex-wrap: wrap;padding-left: 40rpx;">
						<view :class="[item.type=='money'?'tag-item-ll':'tag-item-ls']"  v-for="item in tag_list">{{item.name}}</view>
					</view>
				</view>
				<!-- 说明 -->
				<view class="step-content" style="padding: 18rpx 24rpx;">
					...
					<navigator url="/pages/detail/content?id=13" hover-class="navigator-hover"><view style="color:#FF6600;font-size: 28rpx;">点击查看示例>>></view></navigator>
					<!-- 图片上传 -->
			...
			<view style="margin-top: 20rpx;" >
				...
				<view class="step-content" style="padding: 18rpx 24rpx;">
					...
					<view>
						<navigator url="/pages/detail/content?id=7" hover-class="navigator-hover" ><view style="color:#FF6600">点击查看示例>>></view></navigator>
					</view>
					<view style="display: flex;align-items: flex-start;justify-content: space-between;">
						<view class="images-operation-cotainer" style="padding: 0;">
							<view style="margin-right: 0;width: 21vw;">
								...
								<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">支付宝付款凭证</view>
							</view>
						</view>
						    ...
						    <view class="s-col is-col-24">
						        <view class="" style="margin-top: 0px!important;background-color: #ececec;">
						            ...
						            <view class="is-item-line "  style="padding:5rpx 24rpx;border-radius: 15rpx;background-color: #fff;">
						                <view class="is-item-bd">
											<view class="uni-form-item uni-column" style="width: 100%;">
												<input placeholder-style="font-size:28rpx" style="height:5vw;" class="uni-input" type="number" :value="sendData.ordersn"  :placeholder="'请输入'+product.platform_name+'订单编号'"  @input="BindInput" data-val="sendData|ordersn" >
											</view>
										</view>
						            </view>			
						        </view>
		...
		<view v-show="order.model==2&&isLoading">
			...
			<view style="padding: 24rpx;background-color:#ececec;text-align: center;padding-bottom: 0px;">
				<view class="is-item timer-box" style="background-color: #ececec;" v-if="order.status<2&&order.order_expire_time > 0">
					...
					<view>
						<uni-countdown font-color="#fff" splitor-color="#333" show-hide="none" bgr-color="#333" border-color="#fffff" splitorTextl=":"
						 splitorTextr=":" :timer="order.order_expire_time_str"></uni-countdown>
					</view>
				</view>
			</view>
		...
		<!-- 预约任务 -->
		<view v-show="order.model==4&&isLoading" style="margin-top: 220rpx;">
			<!-- 倒计时 -->
			<view style="padding: 24rpx;background-color:#ececec;text-align: center;padding-bottom: 0px;padding-left:0px;position: fixed;width: 100%;left: 0;top: 0;z-index: 99;">
				<view class="is-item timer-box" style="flex-direction: column;background-color: #ececec;box-sizing: border-box;" v-if="order.status<2&&order.order_expire_time > 0">
					<view class="is-item-hd" style="font-size: 36rpx;font-weight: 900;" v-if="order.check_time==0">
						上传凭证时间（<span style="color: #f00;">不要去淘宝付款</span>）:
					</view>
					<view class="is-item-hd" style="font-size: 36rpx;font-weight: 900;" v-if="order.check_time>0&&nowDate<order.submit_time">
						回访时间（<span style="color: #f00;">不要去淘宝付款</span>）:
					</view>
					<view class="is-item-hd" style="font-size: 36rpx;font-weight: 900;" v-if="order.check_time>0&&nowDate>=order.submit_time">
						距离任务关闭（<span style="color: #f00;">现在为淘宝付款时间</span>）:
					</view>
			...
			<!-- 第一步 -->
			<view style="margin-top: 20rpx;background-color:#ececec;" >	
				<view class="step-title has-bg-white" ><view class="step-fake-btn">1</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">货比三家及找到目标商品</view></view>
				<view class="step-content" style="padding: 18rpx 24rpx;">
					<!-- <view>请确认<span style="color: #FF4B5E;margin:0 10upx;">{{order.buyer_name}}</span>账号是否登录手机{{product.platform_name}}APP</view>
					<view>打开<span style="color: rgb(255, 75, 94);margin: 0px 5px">{{product.platform_name}}</span>APP手动输入关键词 <span style="color: rgb(255, 75, 94);margin: 0px 5px">{{product.product_keywrods.keywords}}</span></view> -->
					<view class="yuyueinfo-box">
						<view class="yuyueinfo-item-box">
							<view class="yuyueitem-container">
								<view>
									下单买号：
								</view>
								<view style="color: rgb(255, 75, 94);">
									{{order.buyer_name}}
								</view>
							</view>
						</view>
						<view class="yuyueinfo-item-box">
							<view class="yuyueitem-container">
								...
								<view style="color: rgb(255, 75, 94);">
									{{product.platform_name}}{{product.product_platform}}
								</view>
							</view>
						</view>
						<view class="yuyueinfo-item-box">
							<view class="yuyueitem-container">
								...
								<view style="color: rgb(255, 75, 94);">
									{{product.product_keywrods.keywords}}
								</view>
							</view>
						</view>
						<view class="yuyueinfo-item-box">
							<view class="yuyueitem-container">
								<view v-if="order.check_time==0">
									<span style="font-weight:900;color: rgb(255, 75, 94);">预约天数{{product.order_days}}天 ,今日为第1天,请找到目标商品后，上传第二步凭证，当前无需付款，不要下单！当前无需付款，不要下单！当前无需付款，不要下单！</span>
								</view>
								<view v-if="order.check_time>0&&nowDate<order.submit_time">
									<span style="font-weight:900;color: rgb(255, 75, 94);">预约天数{{product.order_days}}天 ,今日为第{{order.current_day}}天,第二步凭证提交后,12小时后才可上传第三步预约凭证,{{order.order_expire_time_str}}后才可下单付款！</span>
								</view>
								<view v-if="order.check_time>0&&nowDate>=order.submit_time">
									<span style="font-weight:900;color: rgb(255, 75, 94);">预约天数{{product.order_days}}天 ,当前为下单时间,请前往{{product.platform_name}}下单付款后提交任务</span>
								</view>
							</view>
						</view>
					</view>
					<view style="background-color: #ececec!important;">
						<uni-collapse style="background-color: #ececec;">
							<uni-collapse-item title="任务要点" animation="inner" style="background-color: #ececec;!important">
								<view style="background-color: #ececec;">
									<view class="" style="background-color: #ececec;">
										<view class="is-list2-info has-list2-tip " style="padding-left: 20rpx;">
									        <view class="">
												<view class="has-mgt-10 has-mgb-10 is-grey is-size-15">请手动输入搜索关键词，关键词且勿自定修改并在目标商品的详情中找到以下问题的答案。</view>
												<view class="has-mgt-10 has-mgb-10 is-grey is-size-15">在当前关键词搜索下，找到<span style="font-weight: 900;color:#fe6248">目标商品</span>的同时随机点击2-3个相关的产品进行货比浏览<span style="color:#fe6248">并在目标商品详情中找到以下问题的答案</span>。 <navigator url="/pages/detail/content?id=4" hover-class="navigator-hover"><span class="is-blue">→点击查看示例</span></navigator></view>
												<view class="has-mgt-10 has-mgb-10 is-grey is-size-15">进入目标商品与货比商品详情页至上而下缓缓浏览2分钟以上</view>
			...
			<!-- 第二步 -->
			<view style="margin-top: 20rpx;" class="step-two">
				<view class="step-title has-bg-white" ><view class="step-fake-btn">2</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">商品凭证上传</view></view>
				<!-- 标签 -->
				...
				<view class="step-content" style="padding: 18rpx 24rpx;">
					...
					<view class="images-operation-cotainer">
						<view>
							<view class="up-operation-box"  @tap="uploadPic" data-name="zuji1" v-if="sendData.zuji1 == ''&&!order.product_currencies.zuji1">
								<image src="../../static/images/common/sub.png"  style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							</view>
							<view v-if="sendData.zuji1 != ''"  @tap="uploadPic" data-name="zuji1">
								<image :src="sendData.zuji1" model="scaleToFill"></image>
							</view>
							<view v-if="order.product_currencies.zuji1" @tap="previewImage(order.product_currencies.zuji1)">
								<image :src="order.product_currencies.zuji1" model="scaleToFill"></image>
							</view>
						</view>
						<view>
							<view  class="up-operation-box" @tap="uploadPic" data-name="zuji2" v-if="sendData.zuji2 == ''&&!order.product_currencies.zuji2">
								<image src="../../static/images/common/sub.png" style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							</view>
							<view v-if="sendData.zuji2 != ''" @tap="uploadPic" data-name="zuji2">
								<image :src="sendData.zuji2" model="scaleToFill"></image>
							</view>
							<view v-if="order.product_currencies.zuji2"  @tap="previewImage(order.product_currencies.zuji2)">
								<image :src="order.product_currencies.zuji2" model="scaleToFill"></image>
							</view>
						</view>
						<view>
							<view class="up-operation-box"  @tap="uploadPic" data-name="shop_goods_link1" v-if="sendData.shop_goods_link1 == ''&&!order.product_currencies.shop_goods_link1">
								<image src="../../static/images/common/sub.png"  style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							...
							<view v-if="order.product_currencies.shop_goods_link1" @tap="previewImage(order.product_currencies.shop_goods_link1)">
								<image :src="order.product_currencies.shop_goods_link1" model="scaleToFill"></image>
							</view>
						</view>
						<view>
							<view  class="up-operation-box" @tap="uploadPic" data-name="shop_goods_link2" v-if="sendData.shop_goods_link2 == ''&&!order.product_currencies.shop_goods_link2">
								<image src="../../static/images/common/sub.png" style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							</view>
							<view v-if="sendData.shop_goods_link2 != ''" @tap="uploadPic" data-name="shop_goods_link2">
								<image :src="sendData.shop_goods_link2" model="scaleToFill"></image>
							</view>
							<view v-if="order.product_currencies.shop_goods_link2" @tap="previewImage(order.product_currencies.shop_goods_link2)">
								<image :src="order.product_currencies.shop_goods_link2" model="scaleToFill"></image>
							</view>
						</view>
					</view>
					<!-- 目标答案验证 -->
					<view class=""  v-if="order.check_time<=0">
						<view class="s-list has-bordertb has-mgtb-0" style="background-color: #ececec;">
							<view class="is-item" style="padding:0px;background-color: #ececec;">
								<view class="is-list2-info has-list2-tip " style="padding: 0px;">
									<view class="form-senddata">
					...
					<navigator url="/pages/detail/content?id=6" hover-class="navigator-hover"><view v-if="order.check_time<=0" style="color:#FF6600;font-size: 28rpx;">点击查看示例>>></view></navigator>
				</view>
				<view v-if="order.check_time<=0" class="advance-title" @tap="submitAdvance">提交并等待商家审核查看</view>
			</view>
			<!-- 第三步预售单显示 -->
			<view  class="ys-show " style="margin-top: 20rpx;">
				<view class="step-title has-bg-white" ><view class="step-fake-btn">3</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">预约凭证上传</view></view>
				<!-- 上传图片说明 -->
				<view style="padding: 18rpx 24rpx;" class="mg-left-cn" >
					<view >请根据商家要求上传相关凭证，每次上传均需点提交</view>
					<view >预约期间可以假聊，问大家、查看商品评论、浏览店铺其它商品等，凡是与产品有关的操作都可以当做任务凭证.</view>
					<view >每天一次回访即可，凭证每天提交一次</view>
					<view >预约最后一日才可以去淘宝/天猫下单，预约任务时间结束后留有24小时操作付款时间</view>
				</view>
				<!-- 图片上传 -->
				<view class="images-operation-cotainer" style="padding: 18rpx 24rpx;" >
					<view>
						<view class="up-operation-box"  @tap="uploadPic" data-name="additional_one" v-if="sendData.additional_one == ''&&!order.product_target.additional_one">
							<image src="../../static/images/common/sub.png"  style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							<text style="font-size: 22rpx;">第一日</text>
						</view>
						<view v-if="sendData.additional_one != ''"  @tap="uploadPic" data-name="additional_one">
							<image :src="sendData.additional_one" model="scaleToFill"></image>
						</view>
						<view v-if="order.product_target.additional_one" @tap="previewImage(order.product_target.additional_one.picture)">
							<image :src="order.product_target.additional_one.picture" model="scaleToFill"></image>
						</view>
						<view :class="['img-sm',sendData.additional_one?'':'not-ready']" @tap="submitImage(1)">{{order.product_target.additional_one?"已提交":'提交'}}</view>
					</view>
					<view v-if="product.order_days>1">
						<view  class="up-operation-box" @tap="uploadPic" data-name="additional_two" v-if="sendData.additional_two == ''&&!order.product_target.additional_two">
							<image src="../../static/images/common/sub.png" style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							<text style="font-size: 22rpx;">第二日</text>
						</view>
						<view v-if="sendData.additional_two != ''" @tap="uploadPic" data-name="additional_two">
							<image :src="sendData.additional_two" model="scaleToFill"></image>
						</view>
						<view v-if="order.product_target.additional_two"  @tap="previewImage(order.product_target.additional_two.picture)">
							<image :src="order.product_target.additional_two.picture" model="scaleToFill"></image>
						</view>
						<view :class="['img-sm',sendData.additional_two?'':'not-ready']" @tap="submitImage(2)">{{order.product_target.additional_two?"已提交":'提交'}}</view>
					</view>
					<view v-if="product.order_days>2">
						<view class="up-operation-box"  @tap="uploadPic" data-name="additional_three" v-if="sendData.additional_three == ''&&!order.product_target.additional_three">
							<image src="../../static/images/common/sub.png"  style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							<text style="font-size: 22rpx;">第三日</text>
						</view>
						<view v-if="sendData.additional_three != ''"  @tap="uploadPic" data-name="additional_three">
							<image :src="sendData.additional_three" model="scaleToFill"></image>
						</view>
						<view v-if="order.product_target.additional_three"  @tap="previewImage(order.product_target.additional_three.picture)">
							<image :src="order.product_target.additional_three.picture" model="scaleToFill"></image>
						</view>
						<view :class="['img-sm',sendData.additional_three?'':'not-ready']" @tap="submitImage(3)">{{order.product_target.additional_three?"已提交":'提交'}}</view>
					</view>
					<view v-if="product.order_days>3">
						<view  class="up-operation-box" @tap="uploadPic" data-name="additional_four" v-if="sendData.additional_four == ''&&!order.product_target.additional_four">
							<image src="../../static/images/common/sub.png" style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
							<text style="font-size: 22rpx;">第四日</text>
						</view>
						<view v-if="sendData.additional_four != ''" @tap="uploadPic" data-name="additional_four">
							<image :src="sendData.additional_four" model="scaleToFill"></image>
						</view>
						<view v-if="order.product_target.additional_four" @tap="previewImage(order.product_target.additional_four.picture)">
							<image :src="order.product_target.additional_four.picture" model="scaleToFill"></image>
						</view>
						<view :class="['img-sm',sendData.additional_four?'':'not-ready']" @tap="submitImage(4)">{{order.product_target.additional_four?"已提交":'提交'}}</view>
					</view>
				</view>
			</view>
			<!-- 第四步 -->
			<view style="margin-top: 20rpx;" >
				<view class="step-title has-bg-white" ><view class="step-fake-btn">4</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">提交订单信息</view></view>
				<view class="step-content" style="padding: 18rpx 24rpx;">
					<view >请确认好下单件数、规格、价格、将商品加入购物车</view>
					<view >付款前核对购买账号、收货信息，确认订单无误后再付款</view>
					<view >下单后请填写实际付款金额与正确的订单编号</view>
			...
			*  -->
			<view >
				<view :class="[btnLoading?'activeBtn':'',nowDate>=order.submit_time&&order.check_time>0?'submit-task-button':'gray-task-button']" @tap="submitTask" >{{btnLoading?'提交中...':'提交任务'}}</view>
			</view>
		</view>
		<!-- 特殊路径任务 -->
		<view v-show="order.model==6&&isLoading">
			<!-- 倒计时 -->
			<view style="padding: 24rpx;background-color:#ececec;text-align: center;padding-bottom: 0px;" v-if="order.status<2&&order.order_expire_time > 0">
				<view class="is-item timer-box" style="background-color: #ececec;" >
					<view class="is-item-hd" style="font-size: 36rpx;font-weight: 900;">
						距离提交任务：
					</view>
					<view>
			...
			<view style="margin-top: 20rpx;background-color:#ececec;" >	
				...
				<view class="step-content" style="padding: 18rpx 24rpx;">
					<view>请确认<span style="color: #FF4B5E;margin:0 10upx;">{{order.buyer_name}}</span>账号是否登录手机{{product.platform_name}}APP</view>
					<view v-if="product.product_channel==0">复制下面链接，打开<span style="color: rgb(255, 75, 94);margin: 0px 5px">{{product.platform_name}}</span>APP找到商品</view>
					<view class="tao-link-box" v-if="product.product_channel==0">
						<view>{{product.special_dispose_info.ambush}}</view>
						<view @tap="clipBoardLink(product.special_dispose_info.ambush)">点击复制</view>
					</view>
					<view v-if="product.product_channel>0">点击查看以下图片路径，打开<span style="color: rgb(255, 75, 94);margin: 0px 5px">{{product.platform_name}}</span>APP找到商品</view>
					<view class="example-img-cotainer" v-if="product.product_channel>0">
						<view v-for="item in product.special_dispose_info.channel_step_img.picture" class="example-img-box" @tap="previewImage(item)">
							<image :src="item" model="scaleToFill"></image>
						</view>
					</view>
					<navigator url="/pages/detail/content?id=59" hover-class="navigator-hover"><view style="color:#FF6600" >点击查看示例>>></view></navigator>
				</view>
			</view>
			<!-- 第二步 -->
			<view style="margin-top: 20rpx;" class="step-two">
				...
				<view class="step-title" ><view class="step-fake-btn">2</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">验证目标答案</view></view>
				<!-- 说明 -->
				<view class="step-content" style="padding: 18rpx 24rpx;">
					<!-- 目标答案验证 -->
					<view class="">
					...
					<navigator url="/pages/detail/content?id=6" hover-class="navigator-hover" ><view v-if="order.check_time<=0" style="color:#FF6600">点击查看示例>>></view></navigator>
				</view>
	...
	export default {
		...
		methods:{
			...
			submitAdvance(){//预约任务第一步提交
				...
				this.$Request("task/submitTaskOrder",data).then(res=>{
					uni.showToast({title:res.msg,icon:'none'});
					if (res.code == 200 ) {
						that.loadData();
						that.sendData.zuji1 = "";
						that.sendData.zuji2 = "";
						that.sendData.shop_goods_link1 = "";
						that.sendData.shop_goods_link2 = "";
						uni.hideLoading();
					} 
				});
			},
			...
			loadData:function(){//加载详情数据
				...
				this.$Request("task/getinfo",{order_sn:this.order_sn,uid:this.userid}).then(res=>{
					if (res.code == 200 ) {
						...
						let list = [];
						for(let i in product.addtional_info.additional_money){
							var obj = {
								name:product.addtional_info.additional_money[i],
								type:'money'
							};
							list.push(obj);
						}
						for(let i in product.addtional_info.additional_action){
							var obj = {
								name:product.addtional_info.additional_action[i],
								type:'action'
							};
							list.push(obj);
						}
						...
						if(_self.$refs.countdownView){
							_self.$refs.countdownView.clearTimer();
							_self.$refs.countdownView.resetTimer(_self.order.order_expire_time_str);
						}
			...
			submitTask:function(){// 任务提交 
				...
				if (this.order.model == 1 ) {//垫付任务，实际付款金额，订单编号，实际付款金额必须小于返款金额，图片至少上传3张，验证目标答案		
				...
				}else if(this.order.model==4){//预约任务
					if(this.order.submit_time == 0){
						uni.showToast({
							title:"请先提交第二步凭证信息~",icon:'none'
						})
					...
					// if((this.nowDate<this.order.submit_time)||(this.order.check_time<=0)){
					// 	uni.showToast({title:"任务时间未到，请在"+this.format(this.order.submit_time)+"之后的12小时内提交任务",icon:'none',duration:2000}) 
					// 	return false;
					// }
					if(this.order.target_time<=0){
						uni.showToast({
							title:"请提交回访凭证~",icon:'none'
						})
						return false;
					}
					if(data['postage_money'] === "") {
				...
				}else if(this.order.model==6){//特殊单 
					if(data['answer'] == "") {
						uni.showToast({title:"请输入目标答案",icon:'none'}) 
						// this.focus.answer = true; 
						return false;
				...
				if(this.order.model==4){//预收单
					posturl="task/submitTaskOrderStepThree"
				}
				if(this.order.model==6){//特殊单
					posturl="task/submitTaskSpecial"
				}
