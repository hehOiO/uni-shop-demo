	...
	<view id="taskOpeartion" >
		...
		<view v-show="order.model==2&&isLoading">
			...
			<view style="margin-top: 20rpx;background-color:#ececec;">	
				<view class="step-title has-bg-white" ><view class="step-fake-btn">1</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">视频浏览</view></view>
				<view class="step-content" style="padding: 18rpx 24rpx;">
					<view style="font-size:28rpx;color: #323232;">请确认<span style="color: #FF4B5E;margin:0 10upx;">{{order.buyer_name}}</span>账号是否登录手机{{product.platform_name}}APP</view>
					<view class="tao-link-box" v-if="product.video_search_type==0">
						<view>{{product.goods_link}}</view>
						<view @tap="clipBoardLink(product.goods_link)">点击复制</view>
					</view>
					<!-- <view class="tao-link-box">
						<view>{{product.product_keywrods.comment}}</view>
						<view @tap="clipBoardLink(product.product_keywrods.comment)">点击复制</view>
					</view> -->
					<view class="example-img-cotainer" v-if="product.video_search_type==1">
						<view class="example-img-box" @tap="previewImage(product.douyin_code)">
							<image :src="product.douyin_code" model="scaleToFill"></image>
						</view>
					</view>
					<view style="color: #323232;" v-if="product.video_search_type==0">点击复制链接，打开抖音短视频，点开跳出视频即可浏览</view>
					<view style="color: #323232;" v-if="product.video_search_type==1">点击预览大图，长按保存，打开抖音短视频，点开跳出视频即可浏览</view>
					<view class="set-comment-box" v-if="product.product_keywrods.modeltype==2" style="background-color: rgba(255,255,255,0.4);margin-top: 12rpx;">
						<view style="color: rgb(255, 75, 94);font-weight:900;text-align: center;margin-bottom: 20rpx;">指定文字评论<span style="color:#333;font-weight: 100;">（点击下方文字可复制）</span></view>
						<view style="color: #333;font-weight: 900;text-align: center;" @tap="clipBoardText(product.product_keywrods.comment)">
							{{product.product_keywrods.comment}} 
						</view>
					</view>
					<navigator url="/pages/detail/content?id=93" hover-class="navigator-hover" style="padding: 12rpx;"><view style="color:#FF6600;font-size: 26rpx;">点击查看示例>>></view></navigator>
				</view>
			</view>
			<!-- 第二步 -->
			<view style="margin-top: 20rpx;" class="step-two">
				<view class="step-title has-bg-white" ><view class="step-fake-btn">2</view><view class="" style="padding: 12rpx;margin-left: 20rpx;font-size: 32rpx;">上传凭证</view></view>
				<!-- 标签 -->
				<view style="padding: 18rpx 24rpx;">
					<view class='tag-box-title' style="display: flex;align-items: center;padding: 0px 12rpx;">
					...
					<view style="display: flex;align-items: center;flex-wrap: wrap;padding: 12rpx;padding-top:0px">
						<view class="tag-item-ll">视频浏览</view>
						<view class="tag-item-ls">视频点赞</view>
						<view class="tag-item-ll" v-if="product.product_keywrods.video_follow==1">关注抖音号</view>
						<view class="tag-item-ls" v-if="product.product_keywrods.video_share==1">分享视频</view>
						<view class="tag-item-ll" v-if="product.product_keywrods.video_comment==1">自由评论</view>
						<view class="tag-item-ls" v-if="product.product_keywrods.modeltype==2">指定评论</view>
						<view class="tag-item-ll" v-if="product.product_keywrods.video_comment_thumbs==1">评论点赞</view>
					</view>
				</view>
				<!-- 说明 -->
				<view class="step-content" style="padding: 18rpx 24rpx;">
					...
					<view style="color: rgb(255, 75, 94);">※ 浏览视频请上传视频最后一个画面的截图</view>
					<navigator url="/pages/detail/content?id=92" hover-class="navigator-hover" style="padding: 12rpx;"><view style="color:#FF6600;font-size: 26rpx;">点击查看示例>>></view></navigator>
					<!-- 图片上传 -->
					<view class="images-operation-cotainer1">
						<view>
							...
							<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">浏览/点赞</view>
						</view>
						<view v-if="product.product_keywrods.video_follow==1">
							...
							<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">关注抖音号</view>
						</view>
						<view v-if="product.product_keywrods.video_share==1">
							...
							<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">分享视频</view>
						</view>
						<view v-if="product.product_keywrods.video_comment==1||product.product_keywrods.modeltype==2">
							...
							<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">{{product.product_keywrods.modeltype==2?'指定评论':'自由评论'}}</view>
						</view>
						<view v-if="product.product_keywrods.video_comment_thumbs==1">
							<view class="up-operation-box1"  @tap="uploadPic" data-name="commentThumb" v-if="sendData.commentThumb == ''">
								<image src="../../static/images/common/sub.png"  style="width: 60rpx;height: 60rpx;" mode="scaleToFill"></image>
								<!-- <text>点击上传</text> -->
							</view>
							<view v-if="sendData.commentThumb != ''"  @tap="uploadPic" data-name="commentThumb">
								<image :src="sendData.commentThumb" model="scaleToFill"></image>
							</view>
							<view style="margin-top:10upx;font-size: 22rpx;color:rgb(255, 75, 94);text-align:center;">评论点赞</view>
						</view>	
					</view>
					<!-- 目标答案验证 -->
							...
							<view class="is-item" style="padding:0px;background-color: #ececec;">
								<view class="is-list2-info has-list2-tip " style="padding: 0px;">
									<view class="form-senddata">
										<view style="font-size: 26rpx;padding: 10upx 0;color: #969799;">请在视频评论中找到发布者提供的字句进行验证</view>
					        			<view class="" style="font-size: 26rpx;padding: 10upx 0;color: #969799;">关键字句:{{product.search_keywords}} （{{product.search_keywords_length}}个字符）</view>
					        			<view class="input-answer has-mgt-10">
					        				<input placeholder-style="font-size:26rpx" style="border-color: #ccc;background-color: #fff;" type="text" :value="sendData.answer" :focus="focus.answer" @input="BindInput" data-val="sendData|answer" placeholder="请输入正确的目标答案">
					        				<button @tap="checkAnswer" style="background-color: #FF4B5E;font-size: 28rpx;">验证</button>
					        			</view>
									</view>
					            </view>
							</view>
					...
					<!-- <navigator v-if="product.product_keywrods.isanswer==1" url="/pages/detail/content?id=6" hover-class="navigator-hover" style="padding: 12rpx;"><view v-if="isLoading" style="color:#FF6600;font-size: 26rpx;">点击查看示例>>></view></navigator> -->
				</view>
		...
		methods:{
			...
			clipBoardLink(link){//复制淘口令
				...
				uni.setClipboardData({
				    ...
				    success: function () {
				    	uni.showToast({
							title: "抖音链接已复制，请打开"+platform_name+"客户端完成任务",
							icon: "none"
						});
				    }
				}); 
			...
			clipBoardText(text){
				...
				uni.setClipboardData({
				    ...
				    success: function () {
				    	uni.showToast({
							title: "指定评论文字已复制，请打开"+platform_name+"客户端完成任务",
							icon: "none"
						});
				    }
				}); 
			...
			submitTask:function(){// 任务提交 
				...
				if(this.order.model==2){//浏览单 判断目标答案和四张图片凭证， 其中搜索商品图为必传
					...
					if(data['searchGoods']==""){
						uni.showToast({title:"请上传视频浏览/点赞图片",icon:'none'});
						return false;
					}
					if (this.product.product_keywrods.video_follow == 1 && data['likegoods'] == "") {
						uni.showToast({title:"请上传关注抖音号图片",icon:'none'});
						return false;
					}
					if (this.product.product_keywrods.video_share == 1 && data['likeStore'] == "") {
						uni.showToast({title:"请上传分享视频图片",icon:'none'});
						return false;
					}
					if ( this.product.product_keywrods.video_comment == 1 && data['cartgoods'] == "") {
						uni.showToast({title:"请上传自由评论图片",icon:'none'});
						return false;
					} 
					if( this.product.product_keywrods.video_comment_thumbs==1 && data['commentThumb'] =="" ){
						uni.showToast({title:"请上传评论点赞图片",icon:'none'});
						return false;
					}
				}
				if (this.btnLoading) {
				...
				this.$Request("task/submit_douyin",data).then(res=>{
