			...
			<view class="s-col is-col-24" v-if="mark==1">
				...
				<view style="background-color:#fff;padding:24rpx 30rpx;" v-show="modelid=='more'">
					...
					<view class="mode-operation-box">
						<view class="mode-operation-item-text">
							<view>预售任务</view>
							<view class="mode-operation-small">（先收藏加购，预售多天后下单）</view>
						</view>
						<switch style="transform:scale(0.7)" data-model="4" @change="switchChange" color="#F29F14" checked/>
					</view>
					<view class="mode-operation-box">
						<view class="mode-operation-item-text">
							<view>特殊路径任务</view>
							<view class="mode-operation-small">（非关键词搜索，如淘口令）</view>
						</view>
						<switch style="transform:scale(0.7)" data-model="6" @change="switchChange" color="#F29F14" checked/>
					</view>
				</view>
				...
				<view style="padding:24rpx 30rpx;background-color:#fff;">
					<view v-for="(item,index) in buyerlist" :key="index" class="buyer-item-container">
						<view :class="['buyer-account',item.checked?'text-orange':'']">
							...
							<view class="iconfont icon-jingdongbaitiao icon-box" v-if="item.type == 'jd'"></view>
							<view class="iconfont icon-pinduoduo icon-box" v-if="item.type == 'tmall'"></view>
							<view>{{item.name}}</view>
						</view>
			...
			<view class="has-pd-20 is-center">
				<view class="new-task-popup">
					...
					<view class="has-bordert has-pdt-20 is-left has-title-color is-size-20" @tap="toTaskDetail">
						您已经接到了一个新的任务，请在<text class="is-blue">{{task_submit_time}}</text>完成并提交;<text class="is-blue iconfont icon-hand">点击去做任务</text>
					</view>
				</view>
			</view>
		</uni-popup>
