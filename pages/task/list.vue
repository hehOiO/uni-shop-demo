<template>
	<view class="page">
		<view class="head" style="background-color: rgb(245, 247, 246);border-bottom: 2rpx solid #E2E1E0;" v-if="showbar ">
			<segmented-control :current="current" :values="items" @clickItem="onClickItem" :styleType="styleType" :activeColor="activeColor"></segmented-control>
		</view>
	...
	export default {
		...
		methods:{
			...
			loadData:function(hideload){
				...
				this.$Request("task/getTasklist",{status:this.status,uid:this.userid,page:this.page,model:this.model}).then(res=>{
