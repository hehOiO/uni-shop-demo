		...
		<view class="s-page-wrapper">
			<view class="buyer-list-container">
				<view class="buyer-list-item-box" v-if="buyerlist.length > 0 " v-for="(item,index) in buyerlist" :key="index" @tap="toDetail(item.id)">
					...
					<view class="buyer-item-right2" v-if="item.status==3">
						{{item.statusstr}}
					</view>
				</view>
			</view>
			<!-- <view class="is-flex is-justify-center is-align-center">

				<view class="s-list2   has-bordertb">
					<view class="is-item2 is-a has-pdtb-10" v-if="buyerlist.length > 0 " v-for="(item,index) in buyerlist" :key="index" @tap="toDetail(item.id)">
						<view class="is-list2-info has-bordert">
							<view>{{item.name}}<text class="is-red has-floatr ">[{{item.statusstr}}]</text></view>
							<view class="has-desc-color is-size-14">
								{{item.province}}{{item.addres}}{{item.tel}}
							</view>
						</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			addBuyer() {
				uni.navigateTo({
					url: '/pages/ucenter/security/bindBuyer?mode=' + this.mode
				});
			},
			toDetail(id) {
				uni.navigateTo({
					url: '/pages/ucenter/security/BuyerInfo?id=' + id
				});
			}
		},
		onLoad: function(e) {
